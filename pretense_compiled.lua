...
Config = Config or {}
Config.lossCompensation = Config.lossCompensation or 1.1 -- 为拥有较少区域的一方提供优势。设置为0以禁用。
Config.randomBoost = Config.randomBoost or 0.0004 -- 每30分钟改变一次，为建造速度增加一个随机因素，设置为0以禁用。
Config.buildSpeed = Config.buildSpeed or 10 -- 结构和防御建造速度。
Config.supplyBuildSpeed = Config.supplyBuildSpeed or 85 -- 补给直升机和车队建造速度。
Config.missionBuildSpeedReduction = Config.missionBuildSpeedReduction or 0.12 -- 在AI任务情况下减少建造速度。
Config.maxDistFromFront = Config.maxDistFromFront or 129640 -- 距离前线最大距离（米），超过此距离区域将被迫进入低活动状态（导出模式）。

if Config.restrictMissionAcceptance == nil then Config.restrictMissionAcceptance = true end -- 如果设置为true，只有在友好区域内着陆时才能接受任务。

Config.missions = Config.missions or {}
Config.missionBoardSize = Config.missionBoardSize or 15

Config.carrierSpawnCost = Config.carrierSpawnCost or 500 -- 玩家起飞时航母的资源成本，设置为0以禁用限制。
Config.zoneSpawnCost = Config.zoneSpawnCost or 500 -- 玩家起飞时区域的资源成本，设置为0以禁用限制。
...
do
	...
	function Utils.saveTable(filename, data)
		...
		if not io then
			...
			trigger.action.outText('持久化已禁用', 30)
			return
		end
